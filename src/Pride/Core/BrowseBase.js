...
const BrowseBase = function(setup, parent) {
  this.datastore = setup.datastore;
  this.query = setup.query || this.datastore.baseQuery();

  const self = this;
  const requestFunc = setup.requestFunc || this.datastore.runBrowse;
  ...
  const requestResults = (requestedSection) => {
    ...
    if (requestedSection && self.query.toLimitSection().overlaps(requestedSection)) {
