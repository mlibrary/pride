...
const RequestBuffer = function(requestOptions) {
  ...
  const callWithResponse = (data) => {
    ...
    if (requestSuccessful) callThenClear('success');
    if (requestFailed) callThenClear('failure');
  };
  ...
  const sendRequest = () => {
    ...
    request({
      ...
      attempts: safeCall(requestOptions.attempts) || Settings.connection_attempts,
